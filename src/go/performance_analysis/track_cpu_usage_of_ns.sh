...
start_tracking() {
    ...
    output_file="output/csvs/cpu_usage_pids_$CPU_NAMESPACE-$MODE.csv"
    old_output_file="output/csvs/cpu_usage_pids_$CPU_NAMESPACE-$MODE-old.csv"
    echo "Creating output file: $output_file"

    # If the output file exists, write its content into old_output_file
...
if [ "$run_all" = false ]; then
    ...
    TRACK=$(declare -f start_tracking)

    sudo bash -c "$TRACK; start_tracking 'server_ns' '$1'" &
    SERVER_PID_BACK=$!

    sudo bash -c "$TRACK; start_tracking 'relay_ns' '$1'" &
    RELAY_PID_BACK=$!
    
    sudo bash -c "$TRACK; start_tracking 'client_ns' '$1'" &
    CLIENT_PID_BACK=$!

    wait $SERVER_PID_BACK
    wait $RELAY_PID_BACK
    wait $CLIENT_PID_BACK

    echo "Finished tracking CPU usage of the Go processes in the namespaces."
fi
